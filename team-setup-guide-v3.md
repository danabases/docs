...
1. Why we do this
2. Install the basics — Windows, WSL, Git, VS Code
3. Sign in and verify Copilot access
4. VS Code Profiles — what they are and how we use them
5. Workspaces — cross-team and cross-delivery framework
6. Branch strategy — how we use `main` and branches
...
2. Choose "Sign in with GitHub." Sign in with your **EMU account** (the `name_enterpriseslug` format).
3. Install these extensions (`Ctrl+Shift+X` to open extensions):
   - **GitHub Copilot** (`github.copilot`) — inline suggestions
   - **GitHub Copilot Chat** (`github.copilot-chat`) — chat, agent, `@workspace`
4. Verify: Open a code file, type a function name, wait 2 seconds for a gray suggestion. Then `Ctrl+Alt+I` to open Chat and ask something.
...
2. Is `"github.copilot.chat.codeGeneration.useInstructionFiles": true` in settings?
3. If asking about multiple repos, are they all in a workspace?
4. Verify: In Chat, ask "What instructions do you have for this repo?"
...
2. You can request a bypass through the GitHub UI (this creates an audit log entry — it is not invisible)
3. The bypass is logged and will appear in the compliance dashboard

**If it is a real secret that got into the code by mistake:**
1. Do not bypass — fix the code first
2. Remove the credential from the code
3. Rotate the credential immediately (assume it is compromised)
4. Commit the fix, then push
